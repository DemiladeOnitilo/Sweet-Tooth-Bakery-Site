...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    removeFromCart: (state, action) => {
      ...
      const productToRemove = state.cartItems.find(item => item.id === productIdToRemove);
    
      if (productToRemove) {
        state.cartItems = state.cartItems.filter(item => item.id !== productIdToRemove);

        state.amount -= productToRemove.quantity;
        state.total -= productToRemove.quantity * parseFloat(productToRemove.price);
      }
    },
    updateTotal: (state) =>{
      let newTotal = 0;
    ...
    updateQuantity: (state, action) => {
      ...
      if (productToUpdate) {
        ...
        state.amount = state.cartItems.reduce((acc, item) => acc + item.quantity, 0);
      }
    }
